...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Change Character OPTIONAL',
			"Same as Change Character BUT doesn't do anything\nwhen low quality is enabled.\n\nValue 1: Character to change (Dad, BF, GF)\nValue 2: New character's name"
		],
		[
			'Change Scroll Speed',
		...
		[
			'gfScared',
			'Set to "true" if you want \nGF to start playing the scared animation.\nAny other value will set GF back to normal'
		],
		[
			'newLossMultiplier',
			"Set Value 1 to be the new Health Loss Multiplier (for missing notes)."
		],
		['newHealthGainMultiplier', "Set Value 1 to be the new Health Gain Multiplier."],
		[
			'newDadDrainHealthValue',
			"Set Value 1 to be how much health BF loses \nwhen Dad (opponent) hits a note.\nSet value1 to 'skip' if you don't want to set a new value\nSet Value 2 to 'true' if you want sustain notes\nto deal damage. Set to\n'false' to disable sustain note damage.\n\nWarning: Only works on Hard difficulty or above!"
		],
		[
			'newDodgeDuration',
			"Set Value 1 (float) to be the new Dodge Duration \n(if dodging is allowed)\n(Default Value: 0.222)"
		],
		[
			'newDodgeCooldown',
			"Set Value 1 (float) to be the new Dodge Cooldown \n(if dodging is allowed)\n(Default Value: 0.102)"
		],
		[
			'streetTV state',
		...
		['Green_KB_AlertDouble', "Same as KB_AlertDouble but green."],
		[
			'KB_AttackFire',
		...
		[
			'Gas Effect',
			"Does the gas effect seen in Censory-Overload.\nValue 1 settings:\n'burst'\n'burstFAST'\n'burstFASTER'"
		],
		[
			'Alarm Gradient',
			"Value 1 refers to which side\n('left' or 'right')\nValue 2 is for the alpha to fade to.\nOnly works if Flashing lights is allowed\nand Low Quality is disabled."
		],
		[
			'TerminationIntro',
			"Value 1 refers to which lane (from 0-3). \nSet Value 2 to 'player' to do the player's side."
		],
		[
			'TerminationOutro',
			"Value 1 refers to which lane. \n0-3 = opponent\n4-7 = player"
		],
		[
			'CessationTroll',
			"ONLY WORKS IN CESSATION! \nUsed for the 1/5 chance troll in Cessation. \nSet 'value2' to '1' to force this to always work."
		],
		[
			'TerminateEndEarly',
			"Don't use this please for the love of god\ncuz it'll break a lot of shit otherwise."
		],
		[
			'Rotating',
			"Makes note spin by 22.5 degrees every 2 steps\nif Value 1 is set to '1'.\n \nSet to '2' for every step instead.\nSet to '3' to reset."
		],
	...
	**/
	...
	function addSongUI():Void
	{
		...
		// dodgeEnabled
		var check_dodging = new FlxUICheckBox(loadAutosaveBtn.x, noteSplashesInputText.y + 20, null, null, "Allow dodging", 100);
		...
		curRenderedNotes.forEachAlive(function(note:Note)
		{
			...
			if (note.strumTime <= Conductor.songPosition)
			{
				...
				if (note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1)
				{
					...
					if (!playedSound[data])
					{
						// Updated so that hurt notes don't play sounds.
						if ((playSoundBf.checked && note.mustPress && !note.hitCausesMiss)
							|| (playSoundDad.checked && !note.mustPress && !note.hitCausesMiss))
						{
	...
	function loadJson(song:String):Void
	{
		if (song.toLowerCase() == "interlope")
		{
			...
			var hazardBG:BGSprite = new BGSprite('hazard/inhuman-port/cheat-bg');
			...
			var cheater:BGSprite = new BGSprite('hazard/inhuman-port/cheat', -600, -480, 0.5, 0.5);
			...
			FlxG.sound.play(Paths.sound('hazard/cheatercheatercheater'), 1, true);
			#if desktop
			// Updating Discord Rich Presence
	...
	private function saveEvents()
	{
		...
		var eventsSong:SwagSong = {
			...
			stage: _song.stage,
			validScore: false,
			dodgeEnabled: _song.dodgeEnabled
		};
		var json = {
			"song": eventsSong
		}
