...
class Alphabet extends FlxSpriteGroup
{
	...
	public var dialogueSFX:String = 'generic';

	public var typingSpeed:Float = 0.05;
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1, ?dialogueSFX:String = "generic")
	{
		super(x, y);
	...
	public function startTypedText(speed:Float):Void
	{
		...
		if(speed <= 0) {
			...
			dialogueSound = FlxG.sound.play(Paths.sound('dialogue/'+dialogueSFX));
		} else {
